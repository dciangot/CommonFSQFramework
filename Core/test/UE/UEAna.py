...
#        self.hist["ptak7"] =  ROOT.TH1F("pt_ak7",   "ptTrackJets",  20, 0, 20) 
	...
	n_tot_gen=0.
        ...
        self.hist_vertex["gen_nJets"].Fill(self.fChain.sisCone5ChgGenJetsp4.size())
	...
	for i in  xrange(0, self.fChain.sisCone5ChgGenJetsp4.size()): # SisCone5
		trackp4=self.fChain.sisCone5ChgGenJetsp4.at(i)	
                if self.fChain.sisCone5ChgGenJetsnConst.at(i)>1:
                  self.hist["gen_nJetTracks"].Fill(self.fChain.sisCone5ChgGenJetsnConst.at(i)) 
	...
	self.hist_full_genjet["fgen_ptSisCone5"].Fill(pt_gen)
	self.hist_full_genjet["fgen_phiSisCone5"].Fill(phi_gen)
	self.hist_full_genjet["fgen_etaSisCone5"].Fill(eta_gen)		   	
#	self.hist_full_genjet["gen_nTracks_SisCone5"].Fill(trackjetg.s_number_of_tracks) TO DO
	if not pt_gen < 1. and math.fabs(eta_gen)<2.: # and trackjetg._genJets_number_of_tracks>1 :
	       ...
	       for track in self.fChain.genParticlesp4:
                  ...
                  if track.pt()>0.5 and math.fabs(track.eta())<2. : #and self.genTracks.getSize()>4:
...
#        for i in self.fChain.SisCone5CHp4:
 #           self.hist_full_jet["f_ptSisCone5"].Fill(i.pt())
...
#for i in xrange(0, self.fChain.recoTracksp4.size()):
	...
	for h in self.Trans_SisCon5:
           self.Trans_SisCon5[h].Scale(3/(4*4*math.pi))
	for h in self.other_SisCon5:
           self.other_SisCon5[h].Scale(3/(2*4*math.pi))
