...
TrackRefForJet::TrackRefForJet(const edm::ParameterSet& iConfig, edm::ConsumesCollector & iC) :
  ...
  bsToken_(iC.consumes<reco::BeamSpot>(iConfig.getParameter<edm::InputTag>("bsTag"))),
...
TrackRefForJet::~TrackRefForJet() {  }

bool TrackRefForJet::testTrack(const reco::Track &t) const {
  using std::abs;
...
bool TrackRefForJet::testVertices(const reco::Track &t, const reco::VertexCollection &vtxs, const reco::BeamSpot &hBs) const {
  bool ok = false;
  if (vtxs.size() > 0) {
    ...
    for (reco::VertexCollection::const_iterator it = vtxs.begin(), ed = vtxs.end();
	 it != ed; ++it) {
      if (it->isValid() && !it->isFake() && it->ndof()>4 && std::fabs(it->z()-hBs.z0()) < 10 && std::sqrt(it->x()*it->x()+it->y()*it->y()) < 2) {
        tested++;
//	double vxErr= it->xError();
 //       double vyErr= it->yError();
//	double vzErr= it->zError();

       if ((std::fabs(t.dxy(it->position())/t.d0Error()) < rhoVtx_) &&
          (std::fabs(t.dz(it->position())/t.dzError()) < zetaVtx_)) {
        ok = true; break;
       }
      }
...
bool TrackRefForJet::operator()(const reco::Track &t, const edm::Event &evt) const {
  if (!testTrack(t)) return false;
  if (nVertices_ == 0) return false;
  edm::Handle<reco::VertexCollection> hVtx;
  evt.getByToken(vertexToken_, hVtx);
  edm::Handle<reco::BeamSpot> hBs;
  evt.getByToken(bsToken_, hBs);
  return testVertices(t, *hVtx, *hBs);
}

bool TrackRefForJet::operator()(const reco::Track &t, const reco::VertexCollection &vtxs) const {
  if (!testTrack(t)) return false;
